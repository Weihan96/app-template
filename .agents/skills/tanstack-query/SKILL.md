...
description: TanStack Query (React Query) best practices for data fetching, caching, mutations, and server state management in React applications. Activate when building data-driven React apps with server state, especially when creating or refactoring data fetching logic, setting query/cache strategies, implementing optimistic updates, or integrating SSR/SSG.
---

# TanStack Query Best Practices

Comprehensive guidelines for implementing TanStack Query patterns in React applications.
...
Rule files in the `rules/` directory partially include the following sections:
1. **Explanation** - Why this pattern matters
2. **Bad Example** - Anti-pattern to avoid
3. **Good Example** - Recommended implementation
4. **Context** - When to apply or skip this rule

Some rules currently do not include `Good Example`. See `NOTICE.txt` for the current gap list.
